...
public class ChipPlus {
    ...
    private static String chosenBean(int beanChoice) {
        if(beanChoice == 2){
            ...
            totalCost = totalCost + .5;
            totalIngred++;
            return beanOptions.get(beanChoice) + " beans";
        }
    }
    ...
    private static ArrayList<String> chosenSalsa(int salsaChoice) {
        ...
        if(salsaChoice == 3 || (salsaChoice == 4 && (totalIngred + 3) > 9) || totalIngred == 9){
            salsaReturn.add("no salsa");
        } else if(salsaChoice == 4 && (totalIngred + 3) < 9){
            ...
            if(totalIngred > 0 && totalIngred < 5){
                ...
                while(numIngredToAdd != 0){
                    ...
                    if(!chosenIngred.contains(chosenTopping)){
                        ...
                        totalCost += .5;
                        numIngredToAdd = numIngredToAdd - 1;
                    }
                }
            }
            ingredPrint = chosenIngred.toString().substring(1, chosenIngred.toString().length() - 1);
